...
db = cx_Oracle.connect('dbusrm02/futuremkt@mg05')
data = db.cursor()
...
(select gstgdid,sum(gstkcsl) sl from goodsstock where gstmfid='001601' group by gstgdid) kc, 
...
and popmfid='004701' 
and popuid='00') cx, 
(select gdid,sj from gds_goodspricegrp where grpid='001601' and guid='00') zc 
...
kcdata.to_excel('雄楚京东上传库存.xls',index=False)
sjdata.to_excel('雄楚京东上传售价.xls',index=False)
